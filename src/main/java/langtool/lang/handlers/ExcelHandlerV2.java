...
public class ExcelHandlerV2 implements ILangFileHandler {
			...
			for (int sheetIdx = 0; sheetIdx < sheetCnt; sheetIdx++) {
				...
				if (sheet.getDrawingPatriarch() != null) {
					...
					for (XSSFShape shape : shapes) {
						if (shape instanceof XSSFSimpleShape) {
							String text = ((XSSFSimpleShape) shape).getText();
							long lineWordsCnt = CharacterUnifiyEnum.statsCharacterCnt(params, text);
							wordCnt += lineWordsCnt;
							Long colWordCnt = sheetStatsMap.get("Other");
							if (colWordCnt == null) {
								colWordCnt = 0l;

							}
							colWordCnt += lineWordsCnt;
							sheetStatsMap.put("Other", colWordCnt);
						}
					}
				}
	...
	public File fill(File file, Map<String, String> wordsA, Map<String, String> wordsB) throws Exception {
		...
		FileOutputStream fos = null;
		try {
			FileUtil.checkDoneDir(FILLS_DST);
			String xlsName = file.getAbsolutePath().replace(file.getName(), "");
			xlsName += DONE_PATH + PATH_SPLITER + file.getName();
			xlsName = xlsName.substring(0, xlsName.indexOf(".x"));
			...
			for (int sheetIdx = 0; sheetIdx < sheetCnt; sheetIdx++) {
				...
				for (int i = 0; i <= sheet.getLastRowNum(); i++) {
					...
					if (cell1 != null && !StringUtils.isEmpty(cell1.getRawValue())) {
						continue;
					}

					String words = cell0.getStringCellValue();
					String value = null;
					if (wordsA.containsKey(words)) {
						value = wordsA.get(words);
					} else if (wordsB.containsKey(words)) {
						value = wordsB.get(words);
					}
