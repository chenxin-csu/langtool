...
public class FillServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (!"fill_src".equals(type) && !"fill_dst".equals(type)) {
            ...
            String msg = null;
            File fillSrcDir = new File(savePath + "fill_src" + PATH_SPLITER);
            boolean hasError = false;
            if ("fill_dst".equals(type)) {
            ...
            for (FileItem item : items) {
                ...
                if (("fill_src").equals(type) && !item.getName().endsWith(".docx")) {
                    detail.put("error", "原文件和翻译文件只支持.docx格式哟，可以点击页面上方查看帮助信息。");
                    continue;
                }
                if (("fill_dst").equals(type) && !item.getName().endsWith(".xlsx")) {
                ...
                if ("fill_src".equals(type)) {
                    ...
                    if (fillFile == null) {
                        ...
                        detail.put("url", WORKSPACE_PATH + PATH_SPLITER + request.getSession(true).getId() + PATH_SPLITER + "fill_src"
                                + PATH_SPLITER + TMP_PATH + PATH_SPLITER + fillFile.getName());
                    }
                }
            }
